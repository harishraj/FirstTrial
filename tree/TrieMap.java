...
public class TrieMap {
	/*
	 * Implementation of a trie tree, though I made it faster and more compact for long key strings by building
	 * tree nodes only as needed to resolve collisions. Each letter of a key is the index into the following array. 
	 * Values stored in the array are either a Leaf containing the user's value or another TrieMap node if more than
	 * one key shares the key prefix up to that point. Null elements indicate unused, I.E. available slots.
	 * 
	 * Used for retrieval
	 * Store characters in nodes (not keys)
	 * Search hit : node where search ends has a non-null value
	 * Search may be terminated at an intermediate node
	 * Search miss : reach null link or node where search ends has null value
	 * Insertion : Encounter a null link, create a new node
